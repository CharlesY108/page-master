...
const configGrid = {
  headerCellStyle: { 'background-color': '#f1f5f6', 'border-bottom': '1px #dbdbdb solid' },
  border: true,
  autoResize: true,
...
// 树状grid
const treeGrid = {
  treeConfig: { childrenField: 'children', reserve: true },
  // stripe: true,
  proxyConfig: {
    enabled: true,
    autoLoad: false,
 ...
 */
function getGridOption(gridType: string, apiName: string | undefined, pageOptions: VxeGridProps, params?: any) {
  let options = {} as VxeGridProps
  switch (gridType) {
    case 'pageGrid':
      options = Object.assign(JSON.parse(JSON.stringify(pageGrid)), configGrid)
      if (!apiName) {
        options.proxyConfig!.enabled = false
      }
      break
    case 'treeGrid':
  ...
  if (gridType !== 'noAgentGrid') {
    if (options.proxyConfig && apiName) {
...
/** 主表格 */
export function
mainGridFun(gridType: string, apiName: string | undefined, pageOptions: VxeGridProps, params?: any, isNoNeedOpt?: boolean) {
  ...
  function changeParams(params: any) {
    if (gridOptions.proxyConfig && apiName) {
